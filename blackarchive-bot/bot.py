...
ADMIN_ID = 1279885111
WEBHOOK_URL = os.getenv("WEBHOOK_URL", "https://botblack.vercel.app/webhook")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🛒 Перейти в магазин", web_app=WebAppInfo(url="https://telegram-miniapp-ecru.vercel.app"))]
    ])
...
async def my_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for order_id, order in user_orders.items():
        orders_text += (
            ...
            f"   Сумма: {sum(products[pid]['price'] * qty for pid, qty in order['items'].items())}₽\n\n"
        )
...
bot_app.add_handler(CallbackQueryHandler(catalog_handler, pattern="^catalog$"))
